...
cp MegaMan64Recompiled AppDir/usr/bin/
cp -r assets/ AppDir/usr/bin/
cp recompcontrollerdb.txt AppDir/usr/bin/
cp icons/512.png AppDir/MegaMan64Recompiled.png
cp .github/linux/MegaMan64Recompiled.desktop AppDir/

"./linuxdeploy-$LINUX_DEPLOY_ARCH.AppImage" --appimage-extract
mv squashfs-root/ deploy
./deploy/AppRun --appdir=AppDir/ -d AppDir/MegaMan64Recompiled.desktop -i AppDir/MegaMan64Recompiled.png -e AppDir/usr/bin/MegaMan64Recompiled --plugin gtk
...
echo '    APP_FOLDER_PATH=$APP_FOLDER_PATH ./MegaMan64Recompiled' >> AppDir/AppRun
echo 'else' >> AppDir/AppRun
echo '    cd "$this_dir"/usr/bin/' >> AppDir/AppRun
echo '    ./MegaMan64Recompiled' >> AppDir/AppRun
